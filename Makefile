...
noise_lib_path=~/workspace/noise-rs/build/x86_64-unknown-linux-gnu/noise
cgmath_lib_path=~/workspace/cgmath-rs/build/x86_64-unknown-linux-gnu/cgmath
stb_image_lib_path=lib/rust-stb-image/
link_args="-lglfw"
local_libs=$(shell find src/ -type f -name '*.rs')

libs=-L $(glfw_lib_path) -L $(gl_lib_path) -L $(cgmath_lib_path) -L $(noise_lib_path)
